...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        layer = GRU(units, activation='relu', return_sequences=True, implementation=2, name=f'rnn_{i+1}')(layer)
...
def final_model(input_dim, filters, kernel_size, conv_stride, conv_border_mode, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        rnn = Bidirectional(
                GRU(units, return_sequences=True, implementation=2, name='rnn_{}'.format(i), dropout=0.2), 
            merge_mode='concat')(rnn_input)
        rnn_input = BatchNormalization(name='bn_rnn_{}'.format(i))(rnn)
